# 许可证

**Compound — The Writing System That Grows**

---

## 许可协议

本项目采用 **Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0)** 许可证。

完整协议文本：https://creativecommons.org/licenses/by-nc/4.0/

---

## 你可以

- **使用**：个人学习、研究、非商业项目中自由使用
- **修改**：基于本系统进行改造、定制、二次开发
- **分享**：以任何媒介和格式分享本系统

---

## 条件

- **署名**：必须注明原作者（the 1% Founder Program / Viola），提供许可证链接，并说明是否进行了修改
- **非商业性**：不得将本系统用于商业目的（包括但不限于：作为付费产品出售、嵌入付费服务、用于商业培训课程的核心教材）

---

## 不限制

- 使用本系统创作的内容归你所有，不受本许可证限制
- 你可以用本系统为你的客户创作内容（系统本身不可商用，但产出物可以）

---

## 版权

© the 1% Founder Program. 由 Viola 创建。

如需商业授权，请联系：[Twitter @aspect_viola](https://x.com/aspect_viola)
